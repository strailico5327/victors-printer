	...
	const tagActionButton = document.querySelector("#tag-action-button");
	const tagMenuPanel = document.querySelector("#tag-menu-panel");
	...
	let publishTimer = 0;
	let publishLogLines = [];
	let publishLogBaseHeight = 0;
	let publishLogHeightTarget = 0;
	let publishLogHeightTimer = 0;
	let imageTrashTimer = 0;
	let editorScrollbar = null;
	const metaTransitionMs = 260;
	const publishLogFrameBottomPadding = 12;
	...
	function disableSwupFromPublish() {
	...
	function fitEditorTextarea() {
		...
		const nextHeight = Math.ceil(editorTextarea.scrollHeight);
		editorPane?.style.setProperty("--main-editor-active-height", `${nextHeight}px`);
		publishLogHeightTarget = nextHeight;
	...
	function setDatetimeDigit(input, index, digit) {
		...
		if (datetimeTimeIndexes.includes(nextIndex) && datetimeTimeIndexes.some((timeIndex) => chars[timeIndex]?.toLowerCase() === "x")) {
			for (const timeIndex of datetimeTimeIndexes) {
				chars[timeIndex] = "0";
			}
		}
	...
	function setDatetimeUnknownTime(input) {
		const selectedIndex = getClosestDatetimeDigitIndex(input.selectionStart ?? 0);
		if (!datetimeTimeIndexes.includes(selectedIndex)) {
			return false;
		}
		const chars = input.value.padEnd(datetimeMask.length, "0").split("");
		for (const timeIndex of datetimeTimeIndexes) {
		...
		input.value = normalizeDatetimeValue(chars.join(""));
		input.setSelectionRange(datetimeTimeIndexes[datetimeTimeIndexes.length - 1], datetimeTimeIndexes[datetimeTimeIndexes.length - 1]);
		syncEventIdToDatetime();
		renderAllImages();
		markDirty();
		updateDatetimeCaret();
	...
	function gridRatioLabel(width, height) {
		return `${width}:${height}`;
	}
	...
	function hideTagMenu() {
		tagMenuPanel?.classList.add("float-panel-closed");
		tagActionButton?.setAttribute("aria-expanded", "false");
	}

	function showTagMenu() {
		tagMenuPanel?.removeAttribute("hidden");
		tagMenuPanel?.classList.remove("float-panel-closed");
		tagActionButton?.setAttribute("aria-expanded", "true");
	}

	function toggleTagMenu() {
		if (tagActionButton?.getAttribute("aria-expanded") === "true") {
			hideTagMenu();
		} else {
			showTagMenu();
		}
	...
	function setTagEditing(enabled) {
		tagInput?.toggleAttribute("readonly", !enabled);
		updateTagMenuState();
	}

	function updateTagMenuState() {
		const tag = tagInput?.value.trim() ?? "";
		const editable = !tagInput?.hasAttribute("readonly");
		for (const button of tagMenuButtons) {
			const active =
				button.dataset.tagValue === tag ||
				(button.dataset.tagAction === "new" && editable && !tagMenuButtons.some((item) => item.dataset.tagValue === tag));
			button.classList.toggle("current-theme-btn", active);
	...
	function revealPublishLog() {
		...
		publishLogBaseHeight = Math.ceil(editorPane.getBoundingClientRect().height || 0);
		publishLogHeightTarget = publishLogBaseHeight;
	...
	function hidePublishLog() {
		...
		publishLogBaseHeight = 0;
		window.clearTimeout(publishLogHeightTimer);
		publishLogHeightTimer = 0;
	...
	function measurePublishLogHeight() {
		...
		const lines = Array.from(publishLogFrame.querySelectorAll(".ec-line"));
		const minimumHeight = publishLogBaseHeight || editorPane.getBoundingClientRect().height || 0;
		const maximumHeight = Math.max(minimumHeight, Math.round(window.innerHeight * 0.68));
		const firstLine = lines[0];
		const lastLine = lines.at(-1);
		const topInset = firstLine instanceof HTMLElement ? firstLine.offsetTop : 0;
		const contentBottom =
			lastLine instanceof HTMLElement ? lastLine.offsetTop + lastLine.offsetHeight : pre.scrollHeight;
		const measuredHeight = contentBottom + topInset + publishLogFrameBottomPadding;
		return Math.ceil(Math.min(maximumHeight, Math.max(minimumHeight, measuredHeight)));
	}

	function schedulePublishLogHeightUpdate() {
		if (!editorPane) {
			return;
		}
		window.clearTimeout(publishLogHeightTimer);
		publishLogHeightTimer = window.setTimeout(() => {
			publishLogHeightTimer = 0;
			const nextHeight = measurePublishLogHeight();
			if (!nextHeight || nextHeight <= publishLogHeightTarget) {
				return;
			}
			publishLogHeightTarget = nextHeight;
			editorPane.style.setProperty("--main-editor-active-height", `${nextHeight}px`);
	...
	function appendPublishLog(message) {
		...
		schedulePublishLogHeightUpdate();
		const pre = publishLogFrame?.querySelector("pre");
	...
	async function prepareSafeImageAssets(image) {
		...
		const thumb = await encodeSafeImage(source, 1200, "image/webp", 0.8);
	...
	async function buildDraftZip({ finalState = "ready" } = {}) {
		...
		const files = {
			[`${eventId}.md`]: strToU8(buildEventMarkdown()),
		};
	...
	function restoreDraft(zipBytes) {
		...
		if (tagInput) {
			tagInput.value = parsedMarkdown.data.tag || (Array.isArray(parsedMarkdown.data.tags) ? parsedMarkdown.data.tags[0] || "" : parsedMarkdown.data.tags || "");
			setTagEditing(Boolean(tagInput.value) && !tagMenuButtons.some((button) => button.dataset.tagValue === tagInput.value));
		}
	...
	datetimeInput?.addEventListener("keydown", (event) => {
		...
		if (event.key.toLowerCase() === "x") {
			event.preventDefault();
			setDatetimeUnknownTime(datetimeInput);
			return;
		}
	...
	datetimeInput?.addEventListener("beforeinput", (event) => {
		...
		if (event.inputType === "insertText") {
			...
			} else if ((event.data ?? "").toLowerCase() === "x") {
				setDatetimeUnknownTime(datetimeInput);
			}
		}
	});
	...
	datetimeInput?.addEventListener("paste", (event) => {
		...
		event.preventDefault();
		const pastedTokens = event.clipboardData?.getData("text").replace(/[^0-9x]/g, "");
		if (!pastedTokens) {
			return;
		}
		...
		for (const index of datetimeDigitIndexes) {
			...
			if (!pastedTokens[digitCursor]) {
				break;
			}
			chars[index] = pastedTokens[digitCursor];
			digitCursor += 1;
		}
	...
	tagActionButton?.addEventListener("click", (event) => {
		event.preventDefault();
		event.stopPropagation();
		toggleTagMenu();
		tagInput?.focus();
	...
	for (const button of tagMenuButtons) {
		button.addEventListener("click", () => {
			if (button.dataset.tagAction === "new") {
				if (tagInput) {
					tagInput.value = "";
				}
				setTagEditing(true);
			} else if (tagInput) {
				tagInput.value = button.dataset.tagValue || "";
				setTagEditing(false);
			}
			hideTagMenu();
			markDirty();
	...
	document.addEventListener("pointerdown", (event) => {
		if (!event.target?.closest?.(".meta-tag")) {
			hideTagMenu();
		}
	});
